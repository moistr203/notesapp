...
import { Authenticator, Button, Text, TextField, Heading, Flex, View, Image, Grid, Divider } from "@aws-amplify/ui-react";
...
export default function App() {
  ...
  async function fetchNotes() {
    ...
    await Promise.all(notes.map(async (note) => {
      if (note.image) {
        const link = await getUrl({ path: ({ identityId }) => `media/${identityId}/${note.image}` });
        note.image = link.url;
      }
      return note;
    }));
  ...
  async function createNote(event) {
    ...
    if (newNote.image)
      await uploadData({ path: ({ identityId }) => `media/${identityId}/${newNote.image}`, data: form.get("image") }).result;
    fetchNotes();
    event.target.reset();
  }
      ...
      {({ signOut }) => (
        <Flex className="App" justifyContent="center" alignItems="center" direction="column" width="70%" margin="0 auto">
          ...
          <Grid margin="3rem 0" autoFlow="column" justifyContent="center" gap="2rem" alignContent="center">
            {notes.map((note) => (
              <Flex key={note.id} direction="column" justifyContent="center" alignItems="center" gap="2rem" border="1px solid #ccc" padding="2rem" borderRadius="5%" className="box">
                <Heading level={3}>{note.name}</Heading>
                <Text fontStyle="italic">{note.description}</Text>
                {note.image && <Image src={note.image} alt={note.name} style={{ width: 400 }} />}
